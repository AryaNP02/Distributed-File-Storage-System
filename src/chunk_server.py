...
class DFSChunkServer:
    def __init__(self, port, data_dir):
        self.port = port
        # Ensure chunk data directories are inside the data folder
        data_base_dir = os.path.join(os.path.dirname(__file__), '..', 'data')
        if not os.path.exists(data_base_dir):
            os.makedirs(data_base_dir)
        self.data_dir = os.path.join(data_base_dir, data_dir) if not os.path.isabs(data_dir) else data_dir
        self.server_id = None
...
if __name__ == '__main__':
    ...
    port = int(sys.argv[1])
    data_dir = sys.argv[2]
    chunk_server = DFSChunkServer(port, data_dir)
